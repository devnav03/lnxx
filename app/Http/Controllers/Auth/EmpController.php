...
class EmpController extends Controller
{
    ...
    */
    ...
    {
        return view('employee.layouts.login');
    }
    ...
    //   dd(\Hash::make($request->password));
       ...
       $ip = $request->getClientIp();
        if (!\Request::ajax()) {             
            ...
            if ($this->auth->attempt($request->only('email', 'password') + ['status' => 1, 'user_type' => 4]) || $this->auth->attempt($credentials + ['user_type' => 4] ))
            {
                if (isEmp()) {
           ...
           else if ($this->auth->attempt($request->only('email', 'password') + ['status' => 1, 'user_type' => 5]) || $this->auth->attempt($credentials + ['user_type' => 5] ))
            {
                if (isWareHouse()) {
                    $LoginLog = new LoginLog();
                    $LoginLog->username = $request->email;
                ...
                $LoginLog->ip = $ip;
                $LoginLog->save();
                return redirect('/employee/lnxx')->with('error', lang('auth.failed_login'));
            }
        }
        else{
            $validator = (new User)->validateLoginUser($credentials);
...
//               $message->to($email);
//               $message->subject('Puka Creations - OTP Login');
//             });
